...
float Shape::PdfLight(int lightsCount)
{
	Sphere* sphere = static_cast<Sphere*>(this);

	float areaOfLightSphere = 4 * PI * powf(sphere->radius, 2);
	float denominator = areaOfLightSphere * static_cast<float>(lightsCount);

	return 1.0f / denominator;
}

vec3 Shape::SampleBRDF(vec3 normal)
{
...
vec3 Shape::EvalScattering(vec3 normal, vec3 omega)
{
	return PdfBRDF(normal, omega) * material->Kd;
}

float Shape::PdfBRDF(vec3 normal, vec3 omega)
{
	return abs(dot(normal, omega)) / PI;
}
...
Intersection Sphere::SampleSphere()
{
	...
	result.normal = normalize(vec3(r * cosf(a), r * sinf(a), z));
