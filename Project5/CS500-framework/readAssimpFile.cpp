...
void recurseModelNodes(Scene* scene,
	...
	std::vector<Material*>,
	const bool activeBlur,
	const int level = 0);

void Scene::ReadAssimpFile(const std::string& path, const bool activeBlur, const mat4& M)
{
	...
	for (int i = 0; i < aiscene->mNumMaterials; i++) {
		...
		if (!emit.IsBlack() && diff.IsBlack()) { // An emitter
			...
			newmat = new Material(Kd, Ks, alpha);

			aiString texPath;
			if (AI_SUCCESS == mtl->GetTexture(aiTextureType_DIFFUSE, 0, &texPath)) {
				fs::path fullPath = path;
				fullPath.replace_filename(texPath.C_Str());
				newmat->setTexture(std::string(fullPath.string()));
			}
		}
...
#endif
	recurseModelNodes(this, aiscene, aiscene->mRootNode, modelTr, materials, activeBlur);
}
	...
	const int level)
{
	...
	for (unsigned int m = 0; m < node->mNumMeshes; ++m) {
		...
		meshdata->activeBlur = activeBlur;
