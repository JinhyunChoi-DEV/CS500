...
public:
	...
	Material() : Kd(vec3(1.0, 0.5, 0.0)), Ks(vec3(1, 1, 1)), Kt(0), alpha(1.0), tex(NULL), IOR(0.0f)
	{
		alpha_phong = alpha;
		alpha_other = abs(sqrtf(2.0f / (alpha_phong + 2)));

	}
	Material(const vec3 d, const vec3 s, const float a)
		: Kd(d), Ks(s), Kt(0), alpha(a), tex(NULL), IOR(0.0f)
	{
		alpha_phong = alpha;
		alpha_other = abs(sqrtf(2.0f / (alpha_phong + 2)));
	}
	...
	{
		Kd = o.Kd;  Ks = o.Ks;  alpha = o.alpha;  tex = o.tex;  Kt = o.Kt;
		alpha_phong = o.alpha;
		alpha_other = abs(sqrtf(2.0f / (alpha_phong + 2)));
		IOR = o.IOR;
	}
	...
	Material(const vec3 d, const vec3 s, const float a, const vec3 t, const float IOR_) :
		Kd(d), Ks(s), Kt(t), alpha(a), tex(NULL), IOR(IOR_)
	{
		alpha_phong = alpha;
		alpha_other = abs(sqrtf(2.0f / (alpha_phong + 2)));
	}
...
public:
	...
	void ReadAssimpFile(const std::string& path, const bool activeBlur, const mat4& M);
	...
	void WriteHDRImage(Color* image, int currentPass);
